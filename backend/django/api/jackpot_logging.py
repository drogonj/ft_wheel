...
# Avoid double-initialization if module is reloaded
_initialized = globals().get("_initialized", False)

if not _initialized:
    log_queue = queue.Queue(-1)

    info_path = os.path.join(LOG_DIR, "jackpot_info.log")
    error_path = os.path.join(LOG_DIR, "jackpot_error.log")
    ...
    logger = logging.getLogger("jackpots")
